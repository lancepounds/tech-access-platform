...
from app.models import Event, RSVP, GiftCard, User, Company # Added Company
from app.extensions import db
from app.auth.decorators import decode_token
import stripe
from datetime import datetime # Added datetime

evt_bp = Blueprint("events", __name__) # Removed url_prefix="/events"
...
def create_event():
    ...
    evt = Event(
        ...
        date=event_date, # Use parsed date
...
def rsvp(evt_id):
    ...
    if not payment_source:
        # Note: RSVP is already committed at this point.
        # Depending on business logic, might want to roll back RSVP or handle differently.
        return jsonify({"error": "Payment source is required for RSVP gift card."}), 400
